...
public class SegmentedLog {
    ...
    private TreeMap<Long, LogSegment> startLogIndexSegmentMap = new TreeMap<>();
    ...
    public SegmentedLog(String raftDataDir, int maxSegmentFileSize) {
        ...
        readSegments();
        for (LogSegment segment : startLogIndexSegmentMap.values()) {
    ...
    public RaftProto.LogEntry getEntry(long index) {
        ...
        LogSegment segment = startLogIndexSegmentMap.floorEntry(index).getValue();
    ...
    public long getLastLogIndex() {
        ...
        LogSegment lastSegment = startLogIndexSegmentMap.lastEntry().getValue();
        ...
        for (RaftProto.LogEntry entry : entries) {
            ...
            try {
                if (segmentSize == 0) {
                    ...
                    LogSegment segment = startLogIndexSegmentMap.lastEntry().getValue();
                    if (!segment.isCanWrite()) {
                        ...
                        segment.setRandomAccessFile(FileIOUtils.openFile(logDataDir, newFileName, "r"));
                    }
                }
                ...
                if (isNeedNewSegmentFile) {
                    ...
                    segment.setRandomAccessFile(FileIOUtils.openFile(logDataDir, newSegmentFileName, "rw"));
                    newSegment = segment;
                } else {
                ...
                newSegment.getEntries().add(new LogSegment.Record(
                        newSegment.getRandomAccessFile().getFilePointer(), entry));
                FileIOUtils.writeProtoToFile(newSegment.getRandomAccessFile(), entry);
    ...
    public void truncatePrefix(long newFirstIndex) {
        ...
        while (!startLogIndexSegmentMap.isEmpty()) {
            LogSegment segment = startLogIndexSegmentMap.firstEntry().getValue();
            if (segment.isCanWrite()) {
                break;
            }
            if (newFirstIndex > segment.getEndIndex()) {
                ...
                try {
                    FileIOUtils.closeFile(segment.getRandomAccessFile());
        ...
        while (!startLogIndexSegmentMap.isEmpty()) {
            ...
            try {
                if (newEndIndex == segment.getEndIndex()) {
                    ...
                    segment.setRandomAccessFile(FileIOUtils.openFile(logDataDir, segment.getFileName(), "rw"));
                }
            } catch (IOException ex) {
    ...
    public void readSegments() {
        try {
            List<String> fileNames = FileIOUtils.getSortedFilesInDir(logDataDir, logDataDir);
            for (String fileName : fileNames) {
                ...
                segment.setRandomAccessFile(FileIOUtils.openFile(logDataDir, fileName, "rw"));
    ...
    public RaftProto.LogMetaData readMetaData() {
        ...
        try (RandomAccessFile randomAccessFile = new RandomAccessFile(file, "r")) {
            RaftProto.LogMetaData metadata = FileIOUtils.readProtoFromFile(
    ...
    public void updateMeta(Long currentTerm, Integer votedFor, Long firstLogIndex, Long commitIndex) {
        ...
        try (RandomAccessFile randomAccessFile = new RandomAccessFile(file, "rw")) {
            FileIOUtils.writeProtoToFile(randomAccessFile, metaData);
